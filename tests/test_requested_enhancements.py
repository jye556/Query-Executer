...
class RequestedEnhancementTests(unittest.TestCase):
    ...
    def test_project_version_endpoint_returns_release_metadata(self):
        with patch("app.main._session_user", return_value={"id": 987, "role": "viewer", "csrf_token_hash": "ok"}), \
            ...
            # Mock GitHub API to return v1.0.4 as latest
            import json
        ...
        self.assertEqual(response.json()["version"], "1.0.4")
        self.assertEqual(response.json()["latest_version"], "v1.0.4")
    ...
    def test_version_endpoint_uses_remote_github_release_when_available(self):
        ...
        self.assertEqual(result["latest_version"], "v1.0.5")
        self.assertTrue(result["update_available"])
        self.assertEqual(result["release_url"], "https://github.com/example/project/releases/tag/v1.0.5")
