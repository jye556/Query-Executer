...
(async () => {
    ...
    const context = {
        ...
        fetch: async url => ({
            ok: true,
            async json() { return { version: "1.0.10", latest_version: "1.0.11", update_available: true, release_url: "https://example.test/release/v1.0.11", changelog: [] }; },
        }),
    };
    ...
    assert.equal(elements.get("version-update-message").textContent, "Version v1.0.11 is available.");
    assert.equal(elements.get("version-update-link").href, "https://example.test/release/v1.0.11");
