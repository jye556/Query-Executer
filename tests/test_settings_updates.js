...
(async () => {
    ...
    const context = {
        ...
        fetch: async url => ({
            ok: true,
            async json() { return { version: "1.0.14", latest_version: "1.0.14", update_available: false, release_url: "https://example.test/release/v1.0.14", changelog: [] }; },
        }),
    };
    ...
    assert.equal(elements.get("version-update-link").href, "https://example.test/release/v1.0.14");
